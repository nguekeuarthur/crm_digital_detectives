...
- **Écran de suivi :** `/signatures` (menu _Signatures_) réunit les contrats et leur état — tuiles de comptage, filtres, recherche — et porte les actions : envoyer à la signature (avec choix du niveau et message), annuler une demande en cours, ouvrir le document signé. Un contrat qui attend depuis plus de 7 jours le signale. En mode simulation, deux boutons permettent de faire aboutir ou refuser une demande sans prestataire. Alimenté par `GET /contracts` et `GET /contracts/signature-stats`, tous deux filtrés par la même règle d'accès que le middleware.
